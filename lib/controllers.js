...
const debug = require('debug')('nodebb-plugin-cloudstorage:controller'),
    ...
    // FirebaseStorage = require('firebase/storage'),
...
module.exports = function () {
    ...
    let settings = {
        providerSettings: {
            ...
            firebase: {
                ...
                options: {},
                deleteUnlinkedContent: false,
                logo: '/plugins/nodebb-plugin-cloudstorage/static/images/firebase-seeklogo.com.svg'
            }
        }
    };
    ...
    function mergeSavedWithDefaults(saved) {
        Object.entries(settings.providerSettings).map(provider => {
            switch (provider[0]) {
                ...
                case ('firebase'):
                    provider[1].config = {
                        firebase_configs: saved['firebase-firebase_configs'],
                        gcp_bucket: saved['firebase-gcp_bucket'],
                        gcp_directory: saved['firebase-gcp_directory'],
                        url_prefix: saved['firebase-gcp_url_prefix']
                    };
                    provider[1].options = {};
                    break;
            }
    ...
    return {
        ...
        providersUpload: function (image, fileObject, callback) {
            ...
            switch (settings.activeProvider) {
                ...
                case 'awss3':
                    ...
                    break;
                case 'firebase':
                    this.firebaseUpload(Object.assign(image, {fileObject}))
                        .then(result => {
                            callback(null, {
                                url: result.Location,
                                name: image.name || ''
                        ...
                        .catch(error => {
                            debug('Error in function providersUpload() firebase: ' + error.message);
                            callback(error);
                        });
                    break;
            }
        ...
        firebaseUpload: function (image) {
            let imageFile = image.fileObject.data,
                etag = image.fileObject.etag;
            if (firebaseAdmin.apps.length === 0) {
                firebaseAdmin.initializeApp({
                    credential: firebaseAdmin.credential.cert(settings.providerSettings.firebase.config.firebase_configs)
                });
            }
            if (!image.headers) {
                const ext = image.path.split('.')[1];
            ...
            return new Promise((resolve, reject) => {
                let newFileName = `${uuid()}.${image.path.split('.')[1]}`
                let bucket = firebaseAdmin.storage().bucket(settings.providerSettings.firebase.config.gcp_bucket);
                let file = bucket.file(`${settings.providerSettings.firebase.config.gcp_directory}/${newFileName}`);
                Readable.from(imageFile)
                    .pipe(file.createWriteStream({
                        metadata: {
                            contentType: `image/${image.path.split('.')[1]}`
                        }
                    }))
                    .on('error', (error) => {
                    ...
                    .on('finish', () => {
                        resolve(`${settings.providerSettings.firebase.config.url_prefix}/${settings.providerSettings.firebase.config.gcp_directory}/${newFileName}`);
                    });
            })
                .then(url => {
                    return {Location: url};
